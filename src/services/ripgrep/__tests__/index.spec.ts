...
describe("getBinPath", () => {
	...
	it("resolves ripgrep from the unpacked classic layout when other paths fail", async () => {
		const rg = path.join(appRoot, "node_modules.asar.unpacked/@vscode/ripgrep/bin", binName)
		mockFileExists.mockImplementation(async (p: string) => p === rg)

		expect(await getBinPath(appRoot)).toBe(rg)
	})

	it("returns undefined when ripgrep binary is not found", async () => {
		mockFileExists.mockResolvedValue(false)
